...
class Server:
    ...
    def write_file(self):
        ...
        server = str(re.search('remote .+ \d+', txt_data, flags=0).group())
        server = str(re.search('\d+\.\d+\.\d+\.\d+', server, flags=0).group())

        extra_option = [
            'keepalive 5 30\r\n',  # prevent connection drop due to inactivity timeout
            'connect-retry 2\r\n',
            "route-nopull\r\n",
            "route " + server + " 255.255.255.255 net_gateway\r\n",
            "route 10.0.0.0 255.128.0.0 net_gateway\r\n"
            ]

        # read Chinese IP route from file
        with open("route.txt") as f:
            lines = f.readlines()
            for line in lines:
                extra_option.append(line)

        #default route
        extra_option.append("route 0.0.0.0 128.0.0.0 vpn_gateway\r\n")
        extra_option.append("route 128.0.0.0 128.0.0.0 vpn_gateway\r\n")
...
def refresh_data():
    ...
    # remove udp node
    vpnlist = dict([vpn for vpn in vpnlist.items() if vpn[1].proto in 'tcp'])
    # remove short-time node (less than ~125h)
    vpnlist = dict([vpn for vpn in vpnlist.items() if int(vpn[1].uptime) >= 43200000])
